 ...
 var tableColW = 100; // cell width of canvas table
 var tableRowH = 30; // cell height of canvas table
 ...
 var firstColW = $( "#table-first-col" ).outerWidth( true ) - 1;


  var courses_ = []; // contains the course objects
 var courseData_ = []; // contains the raw data for the courses
 ...
 function Redraw()
 {
    ...
    tableColW = Math.ceil( $( "#cell" ).outerWidth( true ) );
    ...
    tableRowH = Math.max( 24, ( $( "#timesheet" ).height() - 39 ) / 18 );
    ...
    tableRowH = $( "#cell" ).outerHeight( true );
    firstRowH = $( "#table-first-col" ).outerHeight( true ); // height of the first row of the table to determine the offset for the class divs
    firstColW = $( "#table-first-col" ).outerWidth( true );
    console.log( firstColW + " - first col width");
    console.log( firstRowH + " - first row height" );
 ...
 function Class()
    {
        ...
        this.DrawSession = function( sessionNum ) // draws the individual session of a class
            {
               ...
               // var info;
                ...
                {
                    if ( this.sessions_[ sessionNum ].days[ day ] == 1 )
                    {
                        DrawDiv( ConstructSessionDiv( day, this.sessions_[ sessionNum ].dateStart,
                            this.sessions_[ sessionNum ].dateEnd,
                            this.sessions_[ sessionNum ].timeStart,
                            this.sessions_[ sessionNum ].timeEnd,
                            this.style ) );
                    }
        ...
        function ConstructSessionDiv( day, dateStart, dateEnd, timeStart, timeEnd, style )
        {
            ...
            var width = Math.max( 7, Math.min( dateEnd - dateStart, semEnd - semStart ) );
            console.log( width + "width 1");

            dateStart = Math.min( Math.max( dateStart - semStart, 0 ), semEnd - width );
            console.log( dateStart + "dateStart");

            sessionDiv.x = firstColW + ( tableColW ) * day + dateStart - 1;
            console.log( sessionDiv.x + "x");

            sessionDiv.y = firstRowH + ( timeStart ) * ( tableRowH / 60 ) - 1;
            console.log( sessionDiv.y + "y");

            sessionDiv.width = width  * ( tableColW / ( semEnd - semStart ) ) - 1;
            console.log( sessionDiv.width + "width 2");

            sessionDiv.height = ( timeEnd - timeStart ) * ( tableRowH / 60 ) - 1;
            console.log( sessionDiv.height + "height");

            sessionDiv.innerColor = 'hsl(' + ( style.hue ) + ', ' + ( style.sat ) + '%, ' + ( style.lit ) + '%)';
        ...
        function DrawDiv( sessionDiv ) // creates a div over the table to represent a class session
            {
                $div = $( "<div/>",
                {
                    class: "drawClass",
        ...
        function GetDrawX( day, dateStart, dateEnd ) // converts the day of the week and start/end dates into x coords
            {
                var semStart, semEnd;
        ...
        function GetDrawY( time ) // converts class start times into y coords
            {
                return ( firstRowH + Math.round( ( time ) * ( tableRowH / 60 ) ) );
            }
    ...
    {
        $( "#canvas" ).html( "" ); // remove all class divs
    }



    function RefreshDrawDiv() // redraws all the class divs
    {
        ClearCanvas();

        for ( var i = 0 ; i < courses_.length ; i++ )
        {
            for ( var j = 0 ; j < courses_[ i ].classes_.length ; j++ )
            {
                courses_[ i ].classes_[ j ].DrawClass();
            }
        }
    }